...
class SAC(SAC_SB3):
    ...
    def train(self, gradient_steps: int, batch_size: int = 64) -> None:
        ...
        for gradient_step in range(gradient_steps):
            ...
            #critic_loss = 0.5 * sum([F.huber_loss(current_q, target_q_values, delta=1000.0) for current_q in current_q_values]) 
            ...
            raw_actor_loss = (ent_coef * log_prob - min_qf_pi).mean() + alpha * th.abs(th.mean(th.sum(replay_data.actions, dim=-1)))
            log_ratio = 100.0
