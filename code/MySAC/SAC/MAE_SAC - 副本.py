...
class SAC(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        # 保存 state_transformer 创建所需的参数（会被保存到模型文件中）
        self.enc_in = enc_in
        self.dec_in = dec_in
        self.c_out_construction = c_out_construction
        ...
        # actor_transformer 和 critic_transformer 仍在 __init__ 中创建
    ...
    def _setup_model(self) -> None:
        ...
        # 创建 state_transformer（在 _setup_model 中创建，确保加载时使用保存的参数）
        if not hasattr(self, 'state_transformer') or self.state_transformer is None:
            self.state_transformer = Transformer(
                enc_in=self.enc_in, 
                dec_in=self.dec_in, 
                c_out=self.c_out_construction, 
            ...
            ).to(self.transformer_device)
            
            # 如果提供了预训练模型路径，加载预训练权重
            if self.transformer_path is not None:
                state_dict = th.load(self.transformer_path, map_location=self.transformer_device)
                new_state_dict = OrderedDict()
                for k, v in state_dict.items(): 
                    name = k[7:] 
                    new_state_dict[name] = v
                self.state_transformer.load_state_dict(new_state_dict)
                print("Successfully load pretrained model...", self.transformer_path)
            else:
                print("Successfully initialize transformer model...")
            
            # 创建 transformer 的优化器和损失函数
            initial_lr = self.lr_schedule(1) if self.lr_schedule is not None else (
                self.learning_rate if isinstance(self.learning_rate, (int, float)) else 0.0001
            )
            self.transformer_optim = th.optim.Adam(self.state_transformer.parameters(), lr=initial_lr)
